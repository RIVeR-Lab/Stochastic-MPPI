...
class SquarePathTracking(MPPI):
    
    def __init__(self):
        super().__init__()
        
        ### Square path tracking specific variables ###
        self.cfg.half_track_width_m =  5.5/2.
        ...
        self.cfg.x0 = np.array([0.0,-5.0, -3.14])
    ...
    def solve(self):
        ...
        if self.cfg.planner_model == PlannerModel.GAUSSIAN_PROCESS:
            ...
            rollout_gp_square_path_tracking_numba[self.num_control_rollouts,1](
            ...
            # Find costs for each trajectory
            rollout_unicycle_square_path_tracking_numba[self.num_control_rollouts,1](
            ...
            rollout_edd5_square_path_tracking_numba[self.num_control_rollouts,1](
            vrange_d,wrange_d,lambda_weight_d,u_std_d,x0_d,dt_d,self.noise_samples_d,self.u_cur_d,\
            robot_wheelbase_m_d,half_track_width_m_d,center_radius_m_d, center_square_side_len_m_d,\
    ...
    def move_mppi_task_vars_to_device(self):
        ...
        center_square_side_len_m = np.float32(self.cfg.center_square_side_len_m)
        ...
        return vrange_d, wrange_d, lambda_weight_d, u_std_d, x0_d, dt_d,\
            robot_wheelbase_m,half_track_width_m,center_radius_m, center_square_side_len_m,\
            lane_center_deviation_penalty_d,lane_departure_exponential_factor_d,\
            lane_departure_high_penalty_d,desired_speed_d,desired_speed_penalty_d,\
...
def initiate_planner():
    ...
    # Track parameters
    outer_square_side_len = cfg.center_square_side_len_m + cfg.half_track_width_m 
    inner_square_side_len = cfg.center_square_side_len_m - cfg.half_track_width_m 
    ...
    for t in range(cfg.max_steps):
        ...
        if t % cfg.plot_every_n == 0:
            ...
            outer_square_patch = patches.Rectangle((-outer_square_side_len/2, -outer_square_side_len/2), 
                                                    outer_square_side_len, 
                                                    outer_square_side_len, 
                                                    linewidth=4, edgecolor='orange',facecolor='bisque')
            
            inner_square_patch = patches.Rectangle((-inner_square_side_len/2, -inner_square_side_len/2), 
                                                    inner_square_side_len, 
                                                    inner_square_side_len, 
                                                    linewidth=4, linestyle = "--",
                                                    edgecolor='orange', facecolor='white')
            
            center_square_patch = patches.Rectangle((-cfg.center_square_side_len_m/2, -cfg.center_square_side_len_m/2), 
                                                    cfg.center_square_side_len_m, 
                                                    cfg.center_square_side_len_m, 
                                                    edgecolor='black', facecolor='none')
            
            ax.add_patch(outer_square_patch)
            ax.add_patch(inner_square_patch)
            ax.add_patch(center_square_patch)
